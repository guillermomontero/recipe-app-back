...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!userDB) return res.status(400).json({ message: 'Invalid user' });
    
    // If user exist, check password
    if (!bcrypt.compareSync(body.password, userDB.password)) return res.status(400).json({ message: 'Invalid password' });
